...
public class ImageController {
    ...
    public String editImage(@RequestParam("imageId") Integer imageId, Model model, HttpSession session, RedirectAttributes redirectAttributes) {
        ...
        if(isOwner(imageId, session)) {
            ...
            String title = imageService.getImage(imageId).getTitle();
            String errMessage = "Only the owner of the image can edit the image";
            redirectAttributes.addAttribute("editError", errMessage).addFlashAttribute("editError", errMessage);
    ...
    public String deleteImageSubmit(@RequestParam(name = "imageId") Integer imageId, HttpSession session, RedirectAttributes redirectAttributes) {
        if(isOwner(imageId, session)) {
            ...
            String title = imageService.getImage(imageId).getTitle();
            String errMessage = "Only the owner of the image can edit the image";
            redirectAttributes.addAttribute("deleteError", errMessage).addFlashAttribute("deleteError", errMessage);
            return "redirect:/images/" + imageId + '/' + title;
        }
    }
    ...
    //The method to check whether logged in user is actual owner of uploaded image
    //If user is actual owner of image, method returns true, else it returns false
    private boolean isOwner(Integer imageId, HttpSession session) {
        Image image = imageService.getImage(imageId);
        ...
        return userId == loggedUserId ? true: false;
    }
}
